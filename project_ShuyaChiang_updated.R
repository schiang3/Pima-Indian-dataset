...
ggplot(diabetes, aes(x=Outcome, y=BloodPressure, fill=Outcome)) + 
  geom_boxplot()+stat_summary(fun="mean",color='yellow', shape=18)+ggtitle('Diabetes vs BloodPressure')
...
xgb.plot.multi.trees(feature_names = names(tnfeatures), render=FALSE,
                     model = bst)
...
xgb.plot.importance(importance_matrix = importance_3, xlab = "Relative importance")

prediction_3<-as.numeric(pred3> 0.5)
confusionMatrix(table(prediction_3, testlabels),positive='1')
library(pROC)
roc(test$Outcome_2,pred3,plot=TRUE,print.auc=TRUE,col='blue',lwd=4,legacy.awes=TRUE,main="XGBoost - ROC Curve",xlab = "False Positive Rate")
...
logistic <- glm(Outcome ~ Glucose+Pregnancies+BloodPressure+SkinThickness+Insulin+BMI+DiabetesPedigreeFunction+Age, data = train, family = "binomial")
summary(logistic)
logistic_Predictions<-predict(logistic,test)
prediction_4<-as.numeric(logistic_Predictions> 0.5)
confusionMatrix(table(prediction_4, test$Outcome_2),positive='1')
